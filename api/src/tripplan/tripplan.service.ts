...
export class TripPlanService {
   constructor(
      ...
      @InjectRepository(UserSegment) private userSegmentRepository: Repository<UserSegment>,
      @InjectRepository(TripPlan) private tripPlansRepository: Repository<TripPlan>
   ) {}
   ...
   async createTripPlan(
      ...
      const userActualSegments = await this.userSegmentRepository.find({
         where: {
      ...
      const tripSegments = actualSegments.map((seg) => {
         ...
         if(segmentDto.reverse)
            tripSegment.direction = seg.startPoint.name
         else
            tripSegment.direction = seg.endPoint.name
         return tripSegment;
      });
